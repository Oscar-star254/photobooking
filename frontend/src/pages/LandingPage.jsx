...
export default function LandingPage() {
  ...
  useEffect(() => {
    api.get("/reviews")
      .then(r => {
        setLiveReviews(r.data.reviews.slice(0, 3));
        setReviewStats({ total: r.data.total, average: r.data.average });
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="min-h-screen bg-dark-900">
      ...
      <section className="relative min-h-screen flex items-center overflow-hidden">
        ...
        <div className="absolute inset-0" style={{ backgroundImage: `linear-gradient(rgba(212,136,30,0.05) 1px, transparent 1px), linear-gradient(90deg, rgba(212,136,30,0.05) 1px, transparent 1px)`, backgroundSize: "60px 60px" }} />
        ...
        {[...Array(12)].map((_, i) => (
          <div key={i} className="absolute w-1 h-1 bg-brand-400 rounded-full opacity-40"
            style={{ top: `${10 + (i * 7) % 80}%`, left: `${5 + (i * 13) % 90}%`, animationDelay: `${i * 0.3}s` }} />
        ))}
        ...
        <div className="page-container relative z-10 pt-24 pb-16">
          <div className="max-w-4xl">
            ...
            <div className="flex flex-wrap gap-4">
              <Link to="/book" className="btn-primary flex items-center gap-2 text-base px-8 py-4 shadow-lg shadow-brand-500/25">
                Book a Session <ArrowRight className="w-5 h-5" />
              </Link>
              <Link to="/portfolio" className="btn-outline flex items-center gap-2 text-base px-8 py-4 backdrop-blur-sm">
                View Portfolio
              </Link>
            </div>
      ...
      <section className="py-24 relative overflow-hidden">
        ...
        <div className="page-container relative z-10">
          <div className="text-center mb-14">
            ...
            <p className="section-subtitle mx-auto">From intimate portraits to grand weddings, we bring creativity to every shoot.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {SERVICES.map((s) => (
              <div key={s.title} className="relative group bg-dark-700/50 backdrop-blur-sm border border-white/5 rounded-xl p-6 hover:border-brand-500/40 hover:bg-dark-600/80 transition-all duration-300 hover:shadow-lg hover:shadow-brand-500/10">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Link to="/book" className="text-gray-500 hover:text-brand-400 text-sm font-body flex items-center gap-1 transition-colors">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div key={item.step} className="relative">
                {i < 3 && <div className="hidden lg:block absolute top-8 left-full w-full h-px bg-gradient-to-r from-brand-500/30 to-transparent z-10" />}
                <div className="relative bg-dark-700/50 backdrop-blur-sm border border-white/5 rounded-xl p-6 h-full hover:border-brand-500/30 transition-all duration-300">
          ...
          <div className="text-center mb-14">
            ...
            {reviewStats.total > 0 && (
              <p className="text-gray-400 font-body mt-2">
                ⭐ {reviewStats.average} average rating · {reviewStats.total} verified reviews
              </p>
            )}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {displayReviews.map((t, i) => (
              <div key={i} className="relative bg-dark-700/50 backdrop-blur-sm border border-white/5 rounded-xl p-6 hover:border-brand-500/20 transition-all duration-300">
                ...
                <p className="text-gray-300 font-body text-sm leading-relaxed mb-5">"{t.comment || t.text}"</p>
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-gradient-to-br from-brand-500 to-brand-700 flex items-center justify-center shrink-0">
                    <span className="text-white font-bold text-sm">{(t.full_name || t.name)?.charAt(0)}</span>
                  </div>
                  <div>
                    <div className="font-display font-semibold text-white text-sm">{t.full_name || t.name}</div>
                    <div className="text-gray-500 text-xs font-body mt-0.5">{t.package_name || t.role}</div>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="py-24 relative overflow-hidden">
        ...
        <div className="absolute inset-0" style={{ backgroundImage: `linear-gradient(rgba(212,136,30,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(212,136,30,0.03) 1px, transparent 1px)`, backgroundSize: "40px 40px" }} />
        <div className="page-container relative z-10 text-center">
          <div className="max-w-2xl mx-auto">
            ...
            <p className="section-subtitle mx-auto mb-8">Book your session today and let us capture your most precious moments.</p>
            <div className="flex flex-wrap gap-4 justify-center">
              <Link to="/book" className="btn-primary text-base px-10 py-4 inline-flex items-center gap-2 shadow-xl shadow-brand-500/25">
                Book Your Session <ArrowRight className="w-5 h-5" />
              </Link>
              <Link to="/portfolio" className="btn-outline text-base px-8 py-4 inline-flex items-center gap-2">
                View Portfolio
              </Link>
            </div>
          </div>
